defmodule HealthBoard.Updaters.SARSUpdater do
  ...
  alias HealthBoard.Updaters.{SARSUpdater, Helpers, Reseeder}
  ...
  def start_link(args) do
    GenServer.start(SARSUpdater, args, name: __MODULE__)
  end
  ...
  def fetch_header(%{header_api_opts: opts, header: current_header} = state) do
    ...
    case SARSUpdater.HeaderAPI.get(opts) do
  ...
  defp download_data?(%{updated_at: updated_at}, last_header, source) do
    if is_nil(last_header) do
      case source do
        %{last_update_date: %Date{} = date} -> Date.compare(updated_at, date) == :gt
        _source -> true
      end
    else
  ...
  def backup_data(state) do
    ...
    if File.dir?(output_path) do
      ...
      if File.dir?(backup_path) do
        temporary_backup_path = "/tmp/sars_updater_backup"
