defmodule HealthBoard.Updaters.CovidReportsUpdater do
  ...
  def start_link(_args) do
    GenServer.start(CovidReportsUpdater, nil, [])
  end
  ...
  defp fetch_header(state) do
    ...
    case CovidReportsUpdater.HeaderAPI.get() do
      {:ok, header} ->
  ...
  defp seed_data(%{temporary_dir: dir} = state) do
    case Reseeder.reseed(Seeders.SituationReport, base_path: Path.join(dir, "output"), what: :covid_reports) do
      :ok ->
        dir
        |> Path.join("backup")
