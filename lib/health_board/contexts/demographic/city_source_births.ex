defmodule HealthBoard.Contexts.Demographic.CitySourceBirths do
  use Ecto.Schema
  import Ecto.Changeset
  alias HealthBoard.Contexts.Demographic.CitySourceBirths
  alias HealthBoard.Contexts.Geo

  schema "cities_source_births" do
    field :date, :date
    field :year, :integer
    field :week, :integer
  ...
  @doc false
  @spec changeset(%CitySourceBirths{}, map()) :: Ecto.Changeset.t()
  def changeset(city_source_births, attrs) do
    city_source_births
    |> cast(attrs, @cast_attrs)
    |> validate_required([:date, :year, :week, :city_id])
    |> unique_constraint([:date, :city_id])
  end
end
