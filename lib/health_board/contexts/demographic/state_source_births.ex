defmodule HealthBoard.Contexts.Demographic.StateSourceBirths do
  use Ecto.Schema
  import Ecto.Changeset
  alias HealthBoard.Contexts.Demographic.StateSourceBirths
  alias HealthBoard.Contexts.Geo

  schema "states_source_births" do
    field :date, :date
    field :year, :integer
    field :week, :integer
  ...
  @doc false
  @spec changeset(%StateSourceBirths{}, map()) :: Ecto.Changeset.t()
  def changeset(state_source_births, attrs) do
    state_source_births
    |> cast(attrs, @cast_attrs)
    |> validate_required([:date, :year, :week, :state_id])
    |> unique_constraint([:date, :state_id])
  end
end
