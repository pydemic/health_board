defmodule HealthBoard.Contexts.Demographic.CountrySourceBirths do
  use Ecto.Schema
  import Ecto.Changeset
  alias HealthBoard.Contexts.Demographic.CountrySourceBirths
  alias HealthBoard.Contexts.Geo

  schema "countries_source_births" do
    field :date, :date
    field :year, :integer
    field :week, :integer
  ...
  @doc false
  @spec changeset(%CountrySourceBirths{}, map()) :: Ecto.Changeset.t()
  def changeset(country_source_births, attrs) do
    country_source_births
    |> cast(attrs, @cast_attrs)
    |> validate_required([:date, :year, :week, :country_id])
    |> unique_constraint([:date, :country_id])
  end
end
