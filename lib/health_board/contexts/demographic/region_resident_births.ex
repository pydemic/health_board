defmodule HealthBoard.Contexts.Demographic.RegionResidentBirths do
  use Ecto.Schema
  import Ecto.Changeset
  alias HealthBoard.Contexts.Demographic.RegionResidentBirths
  alias HealthBoard.Contexts.Geo

  schema "regions_resident_births" do
    field :date, :date
    field :year, :integer
    field :week, :integer
  ...
  @doc false
  @spec changeset(%RegionResidentBirths{}, map()) :: Ecto.Changeset.t()
  def changeset(region_resident_births, attrs) do
    region_resident_births
    |> cast(attrs, @cast_attrs)
    |> validate_required([:date, :year, :week, :region_id])
    |> unique_constraint([:date, :region_id])
  end
end
