defmodule HealthBoard.Contexts.Morbidities.WeeklyMorbiditiesCases do
  import Ecto.Query, only: [order_by: 2, where: 2, dynamic: 1, dynamic: 2]
  alias HealthBoard.Contexts
  alias HealthBoard.Contexts.Morbidities.WeekMorbidityCases
  alias HealthBoard.Repo

  @type schema :: %WeekMorbidityCases{}

  @schema WeekMorbidityCases
  ...
  defp filter_where(params) do
    Enum.reduce(params, dynamic(true), fn
      ...
      {:week_period, [from, to]}, dynamic -> dynamic([row], ^dynamic and row.week >= ^from and row.week <= ^to)
