defmodule HealthBoard.Release.DataPuller.SARSServer do
  ...
  defp fetch_data do
    case OpenDataSUS.get_sars_source_information() do
      {:ok, source_information} ->
        Logger.info("[sars] Information was obtained")
      ...
      _error ->
        Logger.error("[sars] Failed to get information from API")
        :error
    end
  end
  ...
  defp download_file(source_information) do
  ...
  defp consolidate_and_seed(source_information) do
    ...
    error ->
      Logger.error(
        "[sars] consolidation failed. Reason: " <>
