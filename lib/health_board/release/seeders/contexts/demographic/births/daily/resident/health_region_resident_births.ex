defmodule HealthBoard.Release.Seeders.Contexts.Demographic.HealthRegionResidentBirths do
  require Logger
  alias HealthBoard.Contexts.Demographic.HealthRegionResidentBirths
  alias HealthBoard.Release.Seeders.Seeder

  @batch_size 1_200
  @path "demographic/births/resident/resident_health_regions_births.zip"
  ...
  def seed(opts \\ []) do
    Seeder.seed(@path, HealthRegionResidentBirths, &parse/2, Keyword.put(opts, :batch_size, @batch_size))
  end

  defp parse([date | data], file_name) do
    @fields
    |> Enum.zip(Enum.map(data, &String.to_integer/1))
    |> Map.new()
    |> Map.put(:date, Date.from_iso8601!(date))
    |> Map.put(:health_region_id, String.to_integer(file_name))
  end
end
