defmodule HealthBoardWeb.DashboardLive.Fragments.SarsDashboard.HospitalizationFatalityRateRanking do
          ...
          <table :if={{ Enum.any?(data) }} class="uk-table uk-table-hover uk-table-small">
            ...
            <tbody>
              <tr :for.with_index={{ {element, index} <- data.result.ranking }}>
                <th>{{ index }}</th>
                <th>{{ element.name }}</th>
                <th>{{ Humanize.number element.hospitalization_fatality_rate }}</th>
              </tr>
            </tbody>
          </table>
