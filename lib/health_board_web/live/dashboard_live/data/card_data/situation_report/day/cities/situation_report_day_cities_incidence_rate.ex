defmodule HealthBoardWeb.DashboardLive.CardData.SituationReportDayCitiesIncidenceRate do
  ...
  defp fetch_covid_reports_rate(%{incidence: incidence, location_id: location_id} = day_covid_reports, populations) do
    %{total: population} = Enum.find(populations, %{total: 0}, &(&1.location_id == location_id))
    %{name: day_covid_reports.location_name, incidence_rate: Math.incidence_rate(incidence, population)}
  end
