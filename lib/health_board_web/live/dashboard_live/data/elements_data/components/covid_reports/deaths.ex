defmodule HealthBoardWeb.DashboardLive.ElementsData.Components.CovidReports.Deaths do
  ...
  def daily_epicurve(data, params) do
    with {:ok, sars} <- Components.fetch_data(data, params, @sars_daily_param),
         ...
         {:ok, datasets} <- daily_epicurve_data(sars, sr, date_range) do
      datasets
  ...
  defp daily_epicurve_data(sars, sr, date_range) do
    []
