defmodule HealthBoardWeb.DashboardLive.ElementsData.Components.Deaths do
  ...
  def daily_epicurve(data, params) do
    case Components.fetch_data(data, params, "daily_deaths") do
      {:ok, deaths_list} -> {:ok, {:emit_and_hook, daily_epicurve_data(deaths_list)}}
      :error -> :ok
    end
  end

  defp daily_epicurve_data(deaths_list) do
    today = TimeData.today()
    {from, to} = Enum.reduce(deaths_list, {today, today}, &date_min_and_max/2)
    date_range = Date.range(from, to)
    deaths_list = Enum.map(date_range, &fetch_daily_epicurve_data(&1, deaths_list))
    trend = Math.moving_average(deaths_list)
  ...
  defp fetch_daily_epicurve_data(date, deaths_list) do
    Enum.find_value(deaths_list, 0, &if(Date.compare(&1.date, date) == :eq, do: &1.total))
  end

  @spec monthly_chart(map, map) :: {:ok, {:emit_and_hook, {map, String.t(), map}}} | :ok
  def monthly_chart(data, params) do
    case Components.fetch_data(data, params, "monthly_deaths") do
      {:ok, deaths_list} -> {:ok, {:emit_and_hook, monthly_chart_data(deaths_list)}}
      :error -> :ok
    end
  end

  defp monthly_chart_data(deaths_list) do
    months = fetch_months(deaths_list)

    months
    |> Enum.map(&fetch_monthly_chart_data(&1, deaths_list))
    |> wrap_monthly_chart(months)
  end

  defp fetch_months(deaths_list) do
    %{year: y, month: m} = TimeData.today_yearmonth()
    {{fy, fm}, {ty, tm}} = Enum.reduce(deaths_list, {{y, m}, {y, m}}, &month_min_and_max/2)
  ...
  defp fetch_monthly_chart_data({year, month}, deaths_list) do
    Enum.find_value(deaths_list, 0, &if(&1.year == year and &1.month == month, do: &1.total))
  end

  defp wrap_monthly_chart(data, months) do
    {%{ready?: true}, "chart_data", Charts.line(data, "Óbitos", Enum.map(months, fn {y, m} -> "#{y}-#{m}" end))}
  end
  ...
  def weekly_chart(data, params) do
    case Components.fetch_data(data, params, "weekly_deaths") do
      {:ok, deaths_list} -> {:ok, {:emit_and_hook, weekly_chart_data(deaths_list)}}
      :error -> :ok
    end
  end

  defp weekly_chart_data(deaths_list) do
    weeks = fetch_weeks(deaths_list)

    weeks
    |> Enum.map(&fetch_weekly_chart_data(&1, deaths_list))
  ...
  defp fetch_weekly_chart_data({year, week}, deaths_list) do
    Enum.find_value(deaths_list, 0, &if(&1.year == year and &1.week == week, do: &1.total))
  end

  defp wrap_weekly_chart(data, weeks) do
    {%{ready?: true}, "chart_data", Charts.line(data, "Óbitos", Enum.map(weeks, fn {y, w} -> "#{y}-#{w}" end))}
  end
end
