defmodule HealthBoard.Scripts.DATASUS.Immediates.BotulismConsolidator do
  require Logger
  alias HealthBoard.Contexts.Geo

  @dir Path.join(File.cwd!(), ".misc/sandbox")

  @source_path Path.join(@dir, "sources/registries/botulism.csv")
  @results_dir Path.join(@dir, "immediates/botulism")
  ...
  defp parse_line([date, source_city_id, resident_city_id, age_code, sex, classification, evolution, type], cities) do
    ...
    resident_city = Enum.find(cities, &(div(&1.id, 10) == String.to_integer(resident_city_id)))
    source_city = Enum.find(cities, &(div(&1.id, 10) == String.to_integer(source_city_id)))
...
HealthBoard.Scripts.DATASUS.Immediates.BotulismConsolidator.run()
