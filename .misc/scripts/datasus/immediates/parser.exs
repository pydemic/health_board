defmodule HealthBoard.Scripts.DATASUS.Immediates.Parser do
  require Logger
  ...
  @malaria_from_amazon_columns @common_columns ++ [{"RESULT", :integer, :optional}]
  ...
  @diseases %{
    ...
    "MALA" => {"malaria_from_amazon", @malaria_from_amazon_columns},
  ...
  defp parse_and_append_to_csv(stream, file, columns) do
    ...
    indexes = Enum.map(first_line, &parse_index(first_line, &1))
  ...
  defp sort_file(path) do
    Logger.info("Sorting #{Path.basename(path)}")

    {_result, 0} = System.cmd("sort", ~w[-o #{path} #{path}])
  end
end

HealthBoard.Scripts.DATASUS.Immediates.Parser.run()
