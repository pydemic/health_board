defmodule HealthBoard.Scripts.DATASUS.SINASC.RegionsConsolidator do
  require Logger

  @dir Path.join(File.cwd!(), ".misc/sandbox")

  @sources_dir Path.join(@dir, "sources/sinasc")
  @source_buckets_dir Path.join(@sources_dir, "source_states_births")
  @resident_buckets_dir Path.join(@sources_dir, "resident_states_births")

  @results_dir Path.join(@dir, "results/demographic")
  @source_results_dir Path.join(@results_dir, "source_regions_births")
  @resident_results_dir Path.join(@results_dir, "resident_regions_births")
  ...
  defp consolidate({state_file_name, file_index}, buckets_dir, results_dir) do
    Logger.info("[#{file_index}] Extracting #{state_file_name}")

    region_id = String.first(state_file_name)
    ...
    |> append_to_csv(Path.join(results_dir, "#{region_id}.csv"))
  end
  ...
  defp append_to_csv(region_data, region_file_path) do
    region_file = File.open!(region_file_path, [:append])

    region_data
    |> Enum.sort(&(Date.compare(elem(&1, 0), elem(&2, 0)) == :gt))
    |> Enum.each(&write_line(&1, region_file))

    File.close(region_file)
  end

  defp write_line({date, %{non_metric: non_metric_data, metric: metric_data}}, region_file) do
    IO.write(region_file, Enum.join([date] ++ non_metric_data ++ metric_data, ",") <> "\n")
  end
end

HealthBoard.Scripts.DATASUS.SINASC.RegionsConsolidator.run()
