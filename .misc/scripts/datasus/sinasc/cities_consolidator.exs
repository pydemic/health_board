defmodule HealthBoard.Scripts.DATASUS.SINASC.CitiesConsolidator do
  require Logger

  @dir Path.join(File.cwd!(), ".misc/sandbox")

  @sources_dir Path.join(@dir, "sources/sinasc")
  @resident_buckets_dir Path.join(@sources_dir, "cities_data/resident_cities_data")
  @source_buckets_dir Path.join(@sources_dir, "cities_data/source_cities_data")

  @results_dir Path.join(@dir, "results/demographic")
  @source_results_dir Path.join(@results_dir, "source_cities_births")
  @resident_results_dir Path.join(@results_dir, "resident_cities_births")
  ...
  defp consolidate({city_file_name, file_index}, buckets_dir, results_dir) do
    ...
    |> append_to_csv(Path.join(results_dir, city_file_name))
  end

  defp do_consolidate(data, consolidations) do
    [_city_id, date, year, week | metric_data] = data
  ...
  defp append_to_csv(city_data, city_file_path) do
    city_file = File.open!(city_file_path, [:append])

    city_data
    |> Enum.sort(&(Date.compare(elem(&1, 0), elem(&2, 0)) == :gt))
    |> Enum.each(&write_line(&1, city_file))

    File.close(city_file)
  end

  defp write_line({date, %{non_metric: non_metric_data, metric: metric_data}}, city_file) do
    IO.write(city_file, Enum.join([date] ++ non_metric_data ++ metric_data, ",") <> "\n")
  end
end

HealthBoard.Scripts.DATASUS.SINASC.CitiesConsolidator.run()
